...
const PROJECT_SELECTOR_LEVELS = 10;
...
function projectSelectorModal() {
  const selectedLevel = projectSelectorLevel();
  return `
    <section class="project-selector-modal" role="dialog" aria-modal="true" aria-label="Project Selector">
      <div class="project-selector-card">
        ...
        <div class="project-selector-levels" aria-label="Difficulty level">
          ${Array.from(
            { length: PROJECT_SELECTOR_LEVELS },
            (_, index) => index + 1,
          )
            .map(
              (level) => `
                <button class="${levelButtonClass(level, selectedLevel)}" type="button" data-selector-level="${level}">
                  ${level}
                </button>
              `,
            )
        ...
        <div class="project-selector-grid">
          ${PROJECTS.map(
            (project, index) => `
              <button class="${projectButtonClass(index)}" type="button" data-test-project="${index + 1}">
                <span>${projectNumberForLevel(selectedLevel, index)}</span>
        ...
        <button class="project-selector-close ${state.projectSelectorArea === "close" ? "selected" : ""}" type="button" data-project-selector-close>Close</button>
      </div>
    </section>
  `;
}
...
function projectSelectorLevel() {
  return Math.min(Math.max(state.projectSelectorLevel ?? 1, 1), PROJECT_SELECTOR_LEVELS);
}

function projectNumberForLevel(level, projectIndex) {
  return (level - 1) * PROJECTS.length + projectIndex + 1;
}

function levelButtonClass(level, selectedLevel) {
  return [
    level === selectedLevel ? "active" : "",
    state.projectSelectorArea === "levels" && level === selectedLevel ? "selected" : "",
  ]
    .filter(Boolean)
    .join(" ");
}

function projectButtonClass(index) {
  return state.projectSelectorArea === "projects" && index === state.projectSelectorIndex
    ? "selected"
    : "";
}
