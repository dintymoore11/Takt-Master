...
export function visibleLevels() {
  return Array.from({ length: levelCount() }, (_, index) => levelCount() - index)
}
...
export function advanceTradesOneStep() {
  ...
  const movingTrades = [...state.trades]
    .filter((trade) => !trade.finished && trade.pendingSteps > 0 && !trade.delayedToday)
...
export function moveTradeOneZone(trade) {
  ...
  if (targetZone <= zoneCount()) {
    ...
    if (occupant) {
      ...
      addLog(`${trade.name} waited for ${occupant.name} to clear zone ${targetZone}.`)
      return false
    }
    if (state.zoneWorkThisRound.has(targetZone)) {
      trade.delayedToday = true
      trade.waitReason = 'spacing'
      addLog(`${trade.name} waited because zone ${targetZone} was already worked today.`)
      return false
    }
  }
...
export function tradeBlockedByRoadblock(trade) {
  return state.roadblocks.find(
    (roadblock) =>
      !roadblock.resolved && roadblock.tradeId === trade.id && roadblock.zone <= trade.zone,
  )
}
...
export function maybeSpawnRoadblock() {
  ...
  const candidates = state.trades.filter(
    (trade, index) => index < activeTradeCount() && !trade.finished && trade.zone < zoneCount(),
  )
  ...
  state.roadblocks.push({
    ...
    tradeId: trade.id,
    zone,
    label: randomFrom(['Permit hold', 'Missing material', 'Design answer', 'Inspection issue']),
    visualKey: 'barricade',
    resolved: false,
    delayDays: 0,
  })
...
export function roadblockSpawnChance() {
  return Math.min(0.78, ROADBLOCK_SPAWN_CHANCE + (state.projectRound - 1) * 0.055)
}
...
export function cleanupResolvedRoadblocks() {
  state.roadblocks = state.roadblocks.filter((roadblock) => !roadblock.resolved)
}

export function resolveSelectedRoadblock() {
  if (state.phase !== 'running') return
  const roadblock = state.roadblocks.find((item) => item.zone === state.selectedZone)
  if (!roadblock) {
...
export function pushSelectedTrade(targetTrade = state.trades[state.selectedTradeIndex]) {
  if (state.phase !== 'running') return
  ...
  trade.lastRoll = Math.min(Math.max(trade.lastRoll ?? 0, 0) + 1, Math.max(...state.mode.dice))
...
export function endProject() {
  ...
  state.phase = state.gameOver ? 'gameOver' : state.projectRound >= PROJECTS.length ? 'win' : 'ended'
  state.quitConfirm = false
  state.endActionIndex = 0
  if (state.gameOver) {
    state.scoreName = ''
...
export function tradeForSelectedZonePush() {
  return (
    state.trades.find((trade) => !trade.finished && trade.zone === state.selectedZone) ??
...
export function tradeLocation(trade) {
  const steps = trade.pendingSteps > 0 ? ` · ${trade.pendingSteps} step${trade.pendingSteps === 1 ? '' : 's'} left` : ''
...
export function usesPlanGrid() {
  return ['data-center', 'wind-farm', 'housing', 'solar-farm', 'airport', 'hotel-site'].includes(currentProject().type)
}
...
export function zoneFromPlanPosition(row, column) {
  const clampedRow = Math.max(1, Math.min(Math.ceil(zonesPerLevel() / zoneColumns()), row))
  const clampedColumn = Math.max(1, Math.min(zoneColumns(), column))
  const visualColumn = clampedRow % 2 === 0 ? zoneColumns() - clampedColumn + 1 : clampedColumn
  const bay = Math.min(zonesPerLevel(), (clampedRow - 1) * zoneColumns() + visualColumn)
  return bay
}

export function zonesForLevel(level) {
  const firstZone = (level - 1) * zonesPerLevel() + 1
  const zones = Array.from({ length: zonesPerLevel() }, (_, index) => firstZone + index)
  if (usesPlanGrid()) {
    return Array.from({ length: Math.ceil(zones.length / zoneColumns()) }, (_, rowIndex) => {
      const row = zones.slice(rowIndex * zoneColumns(), rowIndex * zoneColumns() + zoneColumns())
...
export function saveLeaderboardScore() {
  ...
  const name = (state.scoreName || 'AAA').trim().toUpperCase().slice(0, 12) || 'AAA'
  const rows = [
    ...loadLeaderboard(),
...
export function continueFromGameOver() {
  state.phase = leaderboardQualifies(state.totalProfit) ? 'nameEntry' : 'scoreResult'
  notifyRender()
}
